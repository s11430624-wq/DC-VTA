...
- `/edit_doc instruction [attachment_1..3]`：針對 `.md`、`.txt`、`.docx` 輸出修訂清單與修正版
  - 目前內建可選：`gemini-2.5-flash-lite`、`gemini-2.5-flash`、`gemini-2.5-pro`、`gemini-3-flash-preview`、`gemini-3.5-flash`、`gemini-3.1-flash-lite-preview`、`gemini-3.1-pro-preview`
- `/question id`：查看指定題目詳情
- `/add content ... [image]`：新增一題選擇題，可選填題目圖片附件
- `/add_short content rubric [image]`：新增一題短答題，可選填題目圖片附件
...
- `/add` 與 `/add_short` 的 `image` 欄位可上傳 PNG、JPG、GIF 或 WEBP 圖片。
- 圖片網址會存入 `question_bank.metadata.image_url`，題目文字仍保留在 `content`。
- 老師使用 `/open` 發題時，圖片會顯示在 Discord 題目 Embed 裡。
- Teacher Dashboard 的題庫列表、短答批改頁與學生短答頁會顯示同一張題目圖片。
