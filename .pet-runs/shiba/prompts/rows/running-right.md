Create a single horizontal sprite strip for the Codex app digital pet `shiba` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Shiba Inu, compact chibi mascot, orange-and-cream coat, curled tail, small triangular ears, friendly smile.
